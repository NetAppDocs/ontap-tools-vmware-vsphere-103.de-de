...
. Löschen Sie FlexVol Volumes aus dem ONTAP System Manager. Siehe  https://docs.netapp.com/us-en/ontap/volumes/delete-flexvol-task.html["Löschen Sie ein FlexVol Volume"^]. Geben Sie im CLI-Befehl zum Löschen eines Volumes den genauen Namen der FlexVol Volumes an.
. Löschen Sie die SAN-iGroups aus dem ONTAP-Speichersystem im Falle einer iSCSI-Bereitstellung. Siehe  https://docs.netapp.com/us-en/ontap/san-admin/manage-san-initiators-task.html["Zeigen Sie SAN-Initiatoren und -Initiatorgruppen an und verwalten Sie sie"^].
